...
final class Resource_2<T: Identifiable> {
    typealias Collection = [T]
    fileprivate(set) var  resource  = Collection() {didSet{notify()}}
    fileprivate var observers = [(Collection)->Void]()
    func notify() {observers.forEach({$0(resource)})}
    func get   () { /* load from network/databse */ }
    func get(id: T.ID) -> T? {resource.first(where: {$0.id == id})                  }
    func upsert(_ item: T)   {resource = resource.filter {$0.id != item.id} + [item]}
    func delete(id: T.ID)    {resource = resource.filter {$0.id != id}              }
...
func ~<<U>(_ lhs: inout Resource_2<U>, _ obs: @escaping ([U]) -> ()) {
    lhs.observe(obs)
}
...
enum Db {
    static var users = Resource_2<User>()
}
